...
import EvmProviderButton from '../EvmProvider/EvmProviderButton'
...
const HomePageContent: React.FC = () => {
  ...
  }, [isReady, isConnected, status, connectors, chainId, connect, isInitializing, handleSetMessage, handleError])
      ...
      <div className="mx-auto flex w-full max-w-md flex-col px-4 sm:px-6">
        ...
        {initializationError && (
          <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-md">
            <p className="text-red-800 text-sm">Initialization Error: {initializationError.message}</p>
          </div>
        )}
      </div>
    )
    ...
    return (
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div className="flex flex-col items-center">
          <Spinner />
          <p className="mt-4 text-gray-600">{isInitializing ? 'Initializing provider...' : 'Initializing wallet...'}</p>
          {initializationError && (
            <p className="mt-2 text-red-600 text-sm text-center max-w-xs">Error: {initializationError.message}</p>
          )}
        </div>
      </div>
    )
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col px-4 sm:px-6 space-y-8">
      ...
      {(connectionError || initializationError) && (
        <div className="p-4 bg-red-50 border border-red-200 rounded-md">
          ...
          {initializationError && (
            <p className="text-red-700 text-sm mb-1">Initialization: {initializationError.message}</p>
          )}
          {connectionError && <p className="text-red-700 text-sm">Connection: {connectionError.message}</p>}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div className="bg-white p-4 rounded-md shadow-2xl space-y-4">
          <h2 className="flex justify-left font-medium text-xl pb-4">Write Contract</h2>
          <div>
