...
export default function AccountActions({ handleSetMessage }: { handleSetMessage: (message: string) => void }) {
  ...
  return (
    <div className="bg-white p-4 rounded-md shadow-2xl space-y-4">
      <h2 className="font-medium text-xl pb-4">Account actions</h2>

      <div className="flex items-center space-x-2 mb-4">
        <Switch id={switchId} checked={isProviderEnabled} onCheckedChange={setIsProviderEnabled} />
        <Label htmlFor={switchId}>EIP-1193 Provider</Label>
      </div>
      ...
      <fieldset>
        <legend className="font-medium leading-6 text-black">Session key duration</legend>
        <div className="mt-3 space-y-1">
          {sessionMethods.map((sessionMethod) => (
            <div key={sessionMethod.id} className="flex items-center">
              ...
              />
              <label htmlFor={sessionMethod.id} className="ml-3 block text-sm font-medium leading-6 text-gray-900">
                {sessionMethod.title}
              </label>
            </div>
          ))}
