...
const PasskeyRecovery = ({
  ...
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const { getEncryptionSession: _getEncryptionSession, createWallet } = useOpenfort()
...
const Content = ({
  ...
  return (
    <div className="space-y-2">
      <p className="font-medium text-black text-sm">Choose a recovery method for your new wallet:</p>
...
const CreateWalletButton = ({ handleSetMessage }: { handleSetMessage: (message: string) => void }) => {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent>
        <SheetTitle className="mb-4">Create a new wallet</SheetTitle>
