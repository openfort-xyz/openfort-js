...
export function logBetterAuthError(error: unknown, context?: string): void {
  ...
  if (error && typeof error === 'object' && 'message' in error) {
    const errorMessage = (error as { message: string }).message
    console.error(`%c${errorPrefix}: ${errorMessage}`, 'color: #ff6b6b; font-weight: bold;')

    // Log user-friendly message
    const userMessage = AUTH_ERROR_MESSAGES[errorMessage] || errorMessage
    console.error(`%c${userMessage}`, 'color: #ffa500;')
    ...
    console.error(`%c${errorPrefix}:`, 'color: #ff6b6b; font-weight: bold;', error)
  }
}
