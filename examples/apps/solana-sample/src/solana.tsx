...
const CustomSolanaWallet = ({ publicKey }: {
  publicKey: PublicKey
}) => {
    ...
    <div className="w-full">
      ...
      {publicKey ? (
        <div className="flex flex-col pt-2">
          ...
          ><strong>Public Key:</strong> {publicKey.toBase58()}</p>
          <p
            onClick={() => { setBalance(null); fetchBalance(publicKey) }}
          ><strong>Balance:</strong> {balance !== null ? `${balance} SOL` : "Loading..."}</p>
          <div className="flex gap-2 pt-2 opacity-[var(--opacity)]" style={{ "--opacity": sendingTransaction ? 0.5 : 1 } as CSSProperties}>
            <input
              type="text"
          ...
          <div className="w-full pt-2">
            {transactionSignature && (
              <>
                <p className="text-green-500" onClick={() => setTransactionSignature(null)}>
