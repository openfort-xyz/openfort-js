...
import type { MethodPath, Methods } from './types'
 ...
 * ]
 */
export const extractMethodPathsFromMethods = (methods: Methods, currentPath: MethodPath = []) => {
...
export const getMethodAtMethodPath = (methodPath: MethodPath, methods: Methods) => {
  const result = methodPath.reduce<Methods | ((...args: any[]) => any) | undefined>(
    (acc, pathSegment) => (isObject(acc) ? acc[pathSegment] : undefined),
    methods
  )
...
export const formatMethodPath = (methodPath: MethodPath) => methodPath.join('.')
