...
const getDestroyedConnectionMethodCallError = (methodPath: MethodPath) => new PenpalError(
...
const connectRemoteProxy = <TMethods extends Methods>(
  ...
  const handleMessage = (message: Message) => {
    ...
    const {
      callId, value, isError, isSerializedErrorInstance,
    } = message;
  ...
  const remoteProxy = createRemoteProxy((methodPath, args) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      replyHandlers.set(callId, {
        methodPath, resolve, reject, timeoutId,
      });
