...
describe('personalSign', () => {
  it('hashes a plain UTF-8 string and its hex encoding identically', async () => {
    ...
    const fromPlain = await personalSign({ params: [text, account.address], signer, account, eventEmitter })
    const fromHex = await personalSign({ params: [hexOf(text), account.address], signer, account, eventEmitter })
  ...
  it('matches the from address case-insensitively', async () => {
    const result = await personalSign({
      params: ['hello', account.address.toLowerCase()],
      signer,
      account,
      eventEmitter,
    })
  ...
  it('tags the signature as a message so ON_SIGNED_MESSAGE can be told apart from typed data', async () => {
    await personalSign({ params: ['hello', account.address], signer, account, eventEmitter })

    expect(signMessage).toHaveBeenCalledWith(expect.objectContaining({ type: 'message', eventEmitter }))
  })

  it('rejects when the from address is not the account', async () => {
    await expect(
      personalSign({ params: ['hello', '0x0000000000000000000000000000000000000009'], signer, account, eventEmitter })
    ).rejects.toThrow(/from address/)
  })

  it('rejects when the message or address is missing', async () => {
    await expect(personalSign({ params: [undefined, account.address], signer, account, eventEmitter })).rejects.toThrow(
